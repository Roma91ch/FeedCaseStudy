...
class RemoteFeedLoaderTest: XCTestCase {
    ...
    func test_load_deliversErrorOnClientError() {
        ...
        expect(sut, toComleteWith: .failure(RemoteFeedLoader.Error.connectivity), when: {
    ...
    func test_load_deliversErrorOnNon200HTTPResponse() {
        ...
        samples.enumerated().forEach { index, code in
            expect(sut, toComleteWith: .failure(RemoteFeedLoader.Error.invalidData), when: {
    ...
    //MARK: - Helpers
    private func expect(_ sut: RemoteFeedLoader, toComleteWith expectedResult: RemoteFeedLoader.Result, when action: () -> Void, file: StaticString = #file, line: UInt = #line) {
    ...
    private func makeSUT(url: URL = URL(string: "http://some-url.com")!, file: StaticString = #file, line: UInt = #line) -> (sut: RemoteFeedLoader, client: HTTPClientSpy) {
        let client = HTTPClientSpy()
